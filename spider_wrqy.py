	...
	for i in range(len(results)):
		...
		for k in range(total_tr1):
			...
			try:
				...
				1 == 1
		sql2 = "select ID from CG_JDXJC_GYQY where QYMC = '%s'"%(qymc)
		cursor.execute(sql2)
		results2 = cursor.fetchall()
		jdxjcid = results2[0][0]
		total_tr2 = (str(Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]').extract())).count('tr')/2
		for k2 in range(total_tr2):
			jcdw = ''
			jcrq = ''
			jcxm = ''
			wrwnd = ''
			...
			cbbs = ''
			try:
				jcdw = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[1]/text()').extract())[0]
			except:
				1==1

			try:
				jcrq = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[2]/text()').extract())[0]
			except:
				1==1
			try:
				jcxm = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[3]/text()').extract())[0]
			except:
				1==1
			pfkid=str(uuid.uuid4())
			l_uuid=pfkid.split('-')
			...
			sql_gas_output = "INSERT INTO CG_JDXJC_DWB (JDXJCQYID,ID,JCDW,GMT_CREATE)VALUES('%s','%s','%s',to_date(\'%s\','yyyy-MM-dd HH24:MI:SS'))"\
			%(jdxjcid,pfkid,jcdw,timenow)
			try:
				cursor.execute(sql_gas_output)
				conn.commit()
			except:
				1 == 1
			try:
				wrwnd = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[4]/text()').extract())[0]
				
			except:
				1==1
			try:
				pfxz = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[5]/text()').extract())[0]
			except:
				1==1
			try:
				dw = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[6]/text()').extract())[0]
			except:
				1==1
			sql_read1 = "select ID from CG_JDXJC_DWB where JDXJCQYID = '%s' and JCDW = '%s'"%(jdxjcid,jcdw)
			cursor.execute(sql_read1)
			results_dw = cursor.fetchall()

			try:
				outputcode = results_dw[0][0]
				monitortime = datetime.datetime.strptime(jcrq,'%Y-%m-%d')
				monitortime = monitortime.strftime("%Y-%m-%d %H:%M:%S")
				wrwnd = wrwnd
				sjbid=str(uuid.uuid4())
				l_uuid=sjbid.split('-')
				sjbid=''.join(l_uuid)
				sql_insert = "INSERT INTO CG_JDXJC_SJB (ID,JDXJCDWBID,JCYZ,JCSJ,SCND,BZXZ,DW,GMT_CREATE)VALUES('%s','%s','%s',to_date(\'%s\','yyyy-MM-dd HH24:MI:SS'),'%s','%s','%s',to_date(\'%s\','yyyy-MM-dd HH24:MI:SS'))"\
				%(sjbid,outputcode,jcxm,monitortime,wrwnd,pfxz,dw,timenow)
	...
	for i in range(len(results)):
		...
		for k2 in range(total_tr2):
			...
			try:
				...
				1==1
			try:
				zxbz = (Selector(text=response.text).xpath('//tbody[@id="sgjczxsjtbl"]/tr['+str(k2+1)+']/td[7]/text()').extract())[0]
			except:
				1==1
			sql_read1 = "select ID from CG_JDXJC_DWB where JDXJCQYID = '%s' and JCDW = '%s'"%(jdxjcid,jcdw)
			cursor.execute(sql_read1)
			...
			try:
				...
				l_uuid=sjbid.split('-')
				sjbid=''.join(l_uuid)
				sql_insert = "INSERT INTO CG_JDXJC_SJB (ID,JDXJCDWBID,JCYZ,JCSJ,SCND,BZXZ,DW,GMT_CREATE,ZXBZ)VALUES('%s','%s','%s',to_date(\'%s\','yyyy-MM-dd HH24:MI:SS'),'%s','%s','%s',to_date(\'%s\','yyyy-MM-dd HH24:MI:SS'),'%s')"\
				%(sjbid,outputcode,jcxm,monitortime,wrwnd,pfxz,dw,timenow,zxbz)
				cursor.execute(sql_insert)
				conn.commit()
			except:
				1 == 1
